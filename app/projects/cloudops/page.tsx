...
export default function CloudOpsPage() {
  return <main className="cloudops-page">
    <nav><Link href="/#projects">← 返回项目案例</Link><span>正式项目 / 已上线</span><a href="/范文豪-Java全栈开发工程师.pdf">查看简历 →</a></nav>
    <header><div><span>CLOUDOPS / WORK ORDER PLATFORM</span><h1>智能工单与<br />协同处置平台</h1></div><p>面向告警事件、人工报障和运维任务的统一协同平台。围绕事件接入、工单流转、责任分派、SLA、过程留痕和知识复用建立端到端处理闭环，并通过 RAG 辅助工单分类、相似问题检索、处置建议和复盘摘要。</p></header>
    ...
    <footer><Link href="/#projects">← 返回全部项目</Link><span>CloudOps / 正式项目 · 已上线</span><a href="/范文豪-Java全栈开发工程师.pdf">查看 PDF 简历 →</a></footer>
  </main>;
}
